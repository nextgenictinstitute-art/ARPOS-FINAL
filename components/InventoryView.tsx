...
export const InventoryView: React.FC<InventoryViewProps> = ({ products, onUpdate }) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter(p => {
      const targetString = `${p.name} ${p.category} ${p.id}`.toLowerCase();
      return searchParts.every(part => targetString.includes(part));
    });
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-6 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Total Items: {filteredProducts.length}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row w-full md:w-auto gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm w-full sm:w-64 focus:ring-2 focus:ring-cyan-500 outline-none"
              value={searchTerm}
          ...
          <button 
            ...
            className="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm font-medium transition-colors"
          >
      ...
      {isAdding && (
        <div className="p-4 bg-slate-50 border-b border-slate-200 grid grid-cols-1 md:grid-cols-7 gap-4 items-end animate-in fade-in slide-in-from-top-4">
          ...
          <div className="flex gap-2">
            <button onClick={handleAddProduct} className="bg-green-600 text-white p-2 rounded flex-1 font-bold">Save</button>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredProducts.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-slate-400 italic">No products found matching your search.</td>
              </tr>
            ) : (
                  ...
                  {editingId === product.id ? (
                    ...
                    <>
                      <td className="px-6 py-4 font-medium text-slate-900">
                        {product.name}
                        <div className="text-[10px] text-slate-400 font-mono mt-0.5 uppercase">ID: {product.id}</div>
                      </td>
                      <td className="px-6 py-4 text-slate-500">{product.category}</td>
                      <td className="px-6 py-4 text-slate-900 font-bold">Rs. {product.price.toFixed(2)}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => handleEditClick(product)} className="text-slate-400 hover:text-cyan-600 transition-colors">
                          <Edit2 size={18} />
                        </button>
                      </td>
                    </>
